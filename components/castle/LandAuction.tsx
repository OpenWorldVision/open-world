...
export default function LandAuction(props: Props) {
  const { action, isOpen, toggleLandAuction } = props
  ...
  const filterLands = useCallback(() => {
    ...
    if (filterSearchId !== '') {
      tempFilteredLands = tempFilteredLands.filter(
        (land) => land.id === parseInt(filterSearchId, 10)
      )
    }

    if (filterStatus !== -1) {
      tempFilteredLands = tempFilteredLands.filter(
        (land) => land.status === filterStatus
      )
    }
    ...
    if (filterRegions.length !== 0) {
      ...
      filterRegions.forEach((region) => {
        temp = [
          ...temp,
          ...tempFilteredLands.filter((land) => land.region === region),
        ]
      })
  ...
  return (
    <Grid
      ...
      gap={{ base: 0, md: 5 }}
    >
      <GridItem colSpan={{ base: 2, md: 3, xl: 2 }} h="fit-content">
        {/* Filter */}
        <LandFilter
          selectedStatus={filterStatus}
          selectedRegions={filterRegions}
          updateFilterStatus={(selectedStatus) =>
            updateFilterStatus(selectedStatus)
          }
          updateFilterRegions={(selectedFilterRegions) =>
            updateFilterRegions(selectedFilterRegions)
          }
          updateFilterSearchId={(selectedFilterId) =>
            updateFilterSearchId(selectedFilterId)
          }
